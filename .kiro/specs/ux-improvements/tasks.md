...
- [x] 1. Extension — Fix copy password and add toast system
  - [x] 1.1 Add `GET_PASSWORD` handler to `apps/extension/src/entrypoints/background.ts`
    - Parse vault JSON, find item by ID, return `{ password }` or `{ error }`
    - _Requirements: 1.2_

  - [x] 1.2 Create Toast component and state management in extension popup
  ...
  - [x] 1.3 Update `handleCopyPassword` to use real password
...
- [x] 2. Extension — Search/filter and suggested items
  - [x] 2.1 Add search input to `ItemListView`
  ...
  - [x] 2.2 Add "Suggested for this site" section
...
- [x] 3. Extension — Item detail view
  - [x] 3.1 Add `GET_ITEM` handler to background worker
    - Returns full item data (all fields including password, notes, etc.) for a given item ID
    - _Requirements: 3.2_

  - [x] 3.2 Add `"item-detail"` to View type and implement ItemDetailView component
  ...
  - [x] 3.3 Add kind icons and visual improvements to item list
...
- [x] 4. Extension — Device management
  - [x] 4.1 Add device-related background handlers
  ...
  - [x] 4.2 Create `DevicesView` component in extension popup
  ...
  - [x] 4.3 Add "Devices" navigation button to item list header
    - Icon button (📱) next to Lock button in ItemListView header
    - Sets view to `"devices"`
    - _Requirements: 12.1_

- [x] 5. Desktop — Full add item form
  - [x] 5.1 Replace `AddItemModal` with comprehensive form
...
- [x] 6. Desktop — Password generator
  - [x] 6.1 Add `generate_password` Tauri command
  ...
  - [x] 6.2 Add "Generate" button to password field in add/edit forms
...
- [x] 7. Desktop — Delete confirmation
  - [x] 7.1 Add confirmation modal before item deletion
...
- [x] 8. Desktop — Keyboard shortcuts
  - [x] 8.1 Register global keyboard handlers in `App.tsx`
...
- [x] 9. Desktop — Device management view
  - [x] 9.1 Add Tauri commands for device management
  ...
  - [x] 9.2 Create `Devices.tsx` page (`apps/desktop/src/pages/Devices.tsx`)
  ...
  - [x] 9.3 Add navigation to Devices view from VaultList
