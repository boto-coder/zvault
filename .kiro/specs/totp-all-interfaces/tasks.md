...
- [x] 1. Add `generate_totp` Tauri command
  - [x] 1.1 Add `TotpResponse` struct and `generate_totp` command to `apps/desktop/src-tauri/src/main.rs`
    - Returns `{ code, remaining_seconds }`
    - Uses `totp_rs::TOTP` with SHA-1, 6 digits, 30s period
    - Register in `invoke_handler`
    - _Requirements: 6.1–6.5_

  - [x] 1.2 Add `validate_totp_secret` Tauri command
    - Takes `secret: String`, returns `Ok(())` or error string
    - Attempts to construct `TOTP::new(...)` — if it fails, return the error
    - Register in `invoke_handler`
    - _Requirements: 7.1, 7.2_

- [x] 2. Add desktop `TotpDisplay` component
  - [x] 2.1 Create `apps/desktop/src/components/TotpDisplay.tsx`
    - Props: `secret: string`
  ...
  - [x] 2.2 Integrate `TotpDisplay` into `ItemDetail.tsx`
    - Show below the password field for login items with `totpSecret`
    - Don't show when `totpSecret` is null/empty
    - _Requirements: 1.1, 1.7_

- [x] 3. Add TOTP secret field to desktop forms
  - [x] 3.1 Add "TOTP Secret" input to `ItemDetail.tsx` edit mode
  ...
  - [x] 3.2 Add "TOTP Secret" input to `AddItemModal` in `VaultList.tsx`
    - Optional field shown when kind is "login"
    - Same validation as edit mode
    - _Requirements: 2.2, 2.5_

  - [x] 3.3 Update `handleSave` in ItemDetail to include `totpSecret` field
...
- [x] 4. Add `--totp` flag to CLI `get` command
  - [x] 4.1 Add `#[arg(long)] totp: bool` to `Get` command struct
    - _Requirements: 3.1_

  - [x] 4.2 Implement TOTP code generation in `cmd_get`
  ...
  - [x] 4.3 Write a test for CLI TOTP output
...
- [x] 5. Add TOTP display to extension item detail
  - [x] 5.1 Update extension `GENERATE_TOTP` handler to also return `remainingSeconds`
  ...
  - [x] 5.2 Create TOTP display section in extension item detail view
...
- [x] 6. Add TOTP badge and copy button to extension item list
  - [x] 6.1 Update `LIST_ITEMS` response to include `hasTotp` boolean per item
  ...
  - [x] 6.2 Add TOTP indicator and copy button to list items
