...
- [x] 1. Core: NIP-19 Module
  - [x] 1.1 Add `bech32 = "0.11"` to workspace dependencies and zvault-core
    - _Requirements: 4.1, 4.2, 4.3_

  - [x] 1.2 Create `crates/zvault-core/src/nip19.rs` with encode/decode functions
  ...
  - [x] 1.3 Add `pub mod nip19;` to `crates/zvault-core/src/lib.rs`

  - [x] 1.4 Write unit tests for NIP-19 codec
...
- [x] 3. CLI: `device show` and `device export-key` subcommands
  - [x] 3.1 Add `Show` and `ExportKey` variants to `DeviceAction` enum
    - Both take `--vault <path>` argument
    - _Requirements: 6.1, 6.2, 6.3_

  - [x] 3.2 Implement `cmd_device_show`
  ...
  - [x] 3.3 Implement `cmd_device_export_key`
    - Prompt for password, decrypt device sidecar
    - Print warning, then nsec + hex
    - Exit 1 if no device identity
    - _Requirements: 3.6, 5.2, 5.4, 6.2, 6.3_

- [x] 4. Checkpoint: CLI builds, `device show` and `device export-key` work

- [x] 5. Desktop Tauri: Key display and export commands
  - [x] 5.1 Implement `get_device_pubkey` Tauri command
  ...
  - [x] 5.2 Implement `export_device_secret_key` Tauri command
  ...
  - [x] 5.3 Register new commands in Tauri Builder

- [x] 6. WASM: npub encoding export
  - [x] 6.1 Add `encode_npub_from_hex(pubkey_hex: &str) -> Result<String, JsValue>` to zvault-wasm
    - Decode hex → bytes, call `nip19::encode_npub`
    - _Requirements: 1.3, 4.1_

- [x] 7. Extension Background: Key export handler
  - [x] 7.1 Add `GET_DEVICE_PUBKEY` message handler
    - Returns device info with npub (uses WASM `encode_npub_from_hex`)
    - _Requirements: 1.3_

  - [x] 7.2 Add `EXPORT_DEVICE_SECRET_KEY` message handler
    - Requires password in payload, re-verify
    - Load secret from encrypted storage, encode as nsec via WASM
    - Return nsec + hex
    - _Requirements: 3.7, 5.2_

- [x] 8. React UI: Key display and export components
  - [x] 8.1 Create `DevicePubkeyCard` component
    - Shows label, device_id, hex pubkey (truncated + copy), npub (full + copy)
    - Integrate into Devices page "My Device" section
    - _Requirements: 1.1, 1.2, 2.1, 2.2_

  - [x] 8.2 Create `ExportSecretKeyDialog` component
    - Warning text + password input → submit → display nsec + hex + copy
    - 30-second auto-hide countdown
    - "Done" button to dismiss
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5_

  - [x] 8.3 Wire components to desktop (Tauri invoke) and extension (sendMessage)
