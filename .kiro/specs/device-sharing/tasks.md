...
- [ ] 1. Core: Pairing Codec Module (`crates/zvault-core/src/pairing.rs`)
  - [ ] 1.1 Create `pairing.rs` module with PairingPayload struct and PairingType enum
  ...
  - [ ] 1.2 Implement `encode_pairing_code(payload) -> Result<String>`
  ...
  - [ ] 1.3 Implement `decode_pairing_code(code) -> Result<PairingPayload>`
  ...
  - [ ] 1.4 Implement builder functions
  ...
  - [ ] 1.5 Add `vault_id: Uuid` field to `Vault` struct
  ...
  - [ ] 1.6 Add `Error::InvalidPairingCode(String)` variant to error enum
    - Descriptive messages for each validation failure
    - _Requirements: 10.1, 10.2, 10.3, 10.4_

  - [ ] 1.7 Write unit tests for pairing codec
...
- [ ] 2. Checkpoint: Core pairing module compiles and tests pass
  - Run `cargo test -p zvault-core --all-features`
  - Run `cargo clippy --workspace --all-targets --all-features -- -D warnings`

- [ ] 3. Desktop Tauri Backend — Pairing Commands
  - [ ] 3.1 Add `vault_id` to AppState/session if not already tracked
    - Ensure the vault_id is accessible from `session.vault.vault_id`
    - _Requirements: 2.1_

  - [ ] 3.2 Implement `create_invite_code` Tauri command
  ...
  - [ ] 3.3 Implement `create_join_request_code` Tauri command
    - Check vault unlocked, identity exists
    - Call `pairing::create_join_request(identity.pubkey_hex, label)`
    - Encode and return
    - _Requirements: 3.1, 3.2, 3.3, 11.1_

  - [ ] 3.4 Implement `import_pairing_code` Tauri command
  ...
  - [ ] 3.5 Implement `confirm_pairing` Tauri command
    - Check vault unlocked, identity exists
  ...
  - [ ] 3.6 Register new commands with Tauri Builder
    - Add `create_invite_code`, `create_join_request_code`, `import_pairing_code`, `confirm_pairing` to invoke_handler
    - _Requirements: 2.1, 3.1_

  - [ ] 3.7 Ensure device identity generation still works (from previous spec)
...
- [ ] 4. Checkpoint: Desktop backend compiles and pairing commands work
  - Build: `cargo build` in `apps/desktop/src-tauri/`
  - Manually test with `cargo tauri dev` if possible

- [ ] 5. WASM Bindings — Pairing Operations
  - [ ] 5.1 Export pairing codec functions from `crates/zvault-wasm/src/lib.rs`
...
- [ ] 8. React UI — Pairing Flow Components
  ...
  - [ ] 8.3 Create `ImportCodeInput` component
  ...
  - [ ] 8.4 Create `InviteDeviceDialog` component (multi-step)
  ...
  - [ ] 8.5 Create `JoinRequestDialog` component (multi-step)
  ...
  - [ ] 8.8 Wire pairing UI to Tauri invoke (desktop)
  ...
  - [ ] 8.9 Wire pairing UI to extension message passing
    - Same flow as desktop but using `browser.runtime.sendMessage` with pairing message types
    - _Requirements: 2.1–2.7, 3.1–3.6_

- [ ] 9. Checkpoint: End-to-end pairing works in desktop app
  - Test invite flow manually between two instances
  - Test join-request flow manually

- [ ] 10. CLI: Pairing Code Support
  - [ ] 10.1 Add `zvault pair invite` subcommand
    - Prints invite code to stdout
    - _Requirements: 9.4_

  - [ ] 10.2 Add `zvault pair request` subcommand
    - Prints join-request code to stdout
    - _Requirements: 9.4_

  - [ ] 10.3 Add `zvault pair import <code>` subcommand
...
- [ ] 11. Tests — Integration and Property
  - [ ] 11.1 Integration test: full invite flow (A invites B)
  ...
  - [ ] 11.2 Integration test: full join-request flow (B requests, A accepts)
  ...
  - [ ] 11.3 Integration test: backward compat with manual admit
  ...
  - [ ] 11.4 Property test: encode/decode round-trip
    - For all valid payloads: decode(encode(p)) == p
    - _Requirements: 4.1_

  - [ ] 11.5 Property test: decode never panics
    - For all random strings: decode either returns Ok or Err (never panics)
    - _Requirements: 10.4_

  - [ ] 11.6 Unit tests for edge cases
